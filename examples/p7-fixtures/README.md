...
│                                      #   - 2 SS, 4 RS total
│                                      #   - sources cite Smith_J_2024_Synth-J-Oncol..003 from
...
3. `/pilar:pillar-statements P-04` drafts two scientific statements (SS-01 Pivotal Phase 2 efficacy; SS-02 Tolerability) and four reference statements (RS-01 / RS-02 under each SS), sourcing each RS against `Smith_J_2024_Synth-J-Oncol`, `Doe_AB_2023_Synth-Lab-Med`, and `Roe_K_2024_Synth-Clin-Oncol` from the P6 fixture manifest. After drafting, `scripts/detect-gaps.py` runs against the pillar to check for orphans (none in this fixture — every RS cites a manifest-resolvable ref-id). The user marks statements approved. Status moves `narrative-approved` → `statements-approved`.
...
Expected: `0 orphan(s)`. Every RS in this fixture cites at least one manifest-resolvable ref-id (Smith_J_2024_Synth-J-Oncol, Doe_AB_2023_Synth-Lab-Med, or Roe_K_2024_Synth-Clin-Oncol).
...
6. Run `/pilar:pillar-statements P-04` — walk through SS/RS drafting; the manifest's entries should surface as candidate sources. Approve at end. Status → `statements-approved`.
