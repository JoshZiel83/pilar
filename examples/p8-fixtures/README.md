...
├── knowledge-base/
│   └── manifest.md                       # 3 REFs (REF-001..003)
├── pillars/
...
2. `/pilar:run-qc --consolidated consolidated/cd-001.md` runs the three QC subagents in §8 / §6.8 sequence, **all read-only**:
   ...
   - **Fact-Checker** at `operating_context: consolidated-draft` reads the consolidated `cd-001.md` + the three cited source files (REF-001 pivotal trial, REF-002 bispecific summary, REF-003 NCCN excerpt). Files one medium-severity finding (drafting placeholder "N months" still in P-04.SS-01.RS-01) and two low-severity observations (one cross-pillar synthesis dependency note, one positive finding).
