...
**Status:** **v0.1.0** (first tagged release) + **Phases 4 and 5 shipped on `main`** (untagged). The plugin installs from a Claude Code session and scaffolds a fully populated engagement repo via `/pilar:init`. `/pilar:sprint-plan`, `/pilar:sprint-close` (with the §5.3 four-option checkpoint state machine), and `/pilar:sprint-amend` drive the sprint workflow. `/pilar:run-qc` orchestrates the §8 Editor-before-Fact-Checker sequencing — both subagents now perform **real** evaluation under the §4/§8 Independence Contract (gated by static `scripts/context-audit.py` on every commit). KB ingestion (P6), per-pillar drafting (P7), and consolidation + Strategic Reviewer (P8) arrive with subsequent phases — see [IMPLEMENTATION_ROADMAP.md](./IMPLEMENTATION_ROADMAP.md) for the full plan and [CHANGELOG.md](./CHANGELOG.md) for release history. **`/pilar:research`** (issue #11) adds targeted PubMed and ClinicalTrials.gov search inline with drafting; requires the **PubMed** and **Clinical Trials** Claude.ai connectors enabled.
